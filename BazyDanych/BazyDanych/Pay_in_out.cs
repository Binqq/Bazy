...
{
    public partial class Pay_in_out : Form
    {
        string data;
        MySqlConnection cnn;
        public Pay_in_out(MySqlConnection connection, string data)
        {
            this.data = data;
            cnn = connection;
            InitializeComponent();
        ...
        private void buttonPayIn_Click(object sender, EventArgs e)
        {
            string Name, Surname;
            double PESEL;
            float Cash;
            ...
            query2 = "SELECT Account_id FROM accounts   JOIN customers ON Customer_id=customers.Customers_id AND customers.Person_id='" + PESEL + "')";
            cnn.Open();
